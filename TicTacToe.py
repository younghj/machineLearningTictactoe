...
class ExperimentGenerator:
    ...
    def isDone(self, board = 0):
        ...
        for y in range(0,3):
            for x in range(0,3):
                if board[y][x] == 0:
                    done = False #not done if there is an empty space
        ...
        for possibility in possibilities:
            ...
            if Xs == 3 or Os == 3:
                done = True #done if there is a win
...
class PerformanceSystem:
    ...
    def setTrainingRate(self, constant):
        self.trainingRate = constant
    ...
    def evaluateBoard(self,board):
        ...
        return h0 + h1*x1 + h2*x2 + h3*x3 + h4*x4 + h5*x5 + h6*x6
    ...
    def chooseMove(self):
        if self.mode == 1:
            successors = self.board.getSuccessorsX() #previous X moves in order
        else:
            successors = self.board.getSuccessorsO() #previous O moves in order

        print successors
        print "----"

        bestSuccessor = successors[0] #inits to first values
        bestValue = self.evaluateBoard(bestSuccessor) #single float value of hypothesis(weights) and feature values
                                                      #input is previous board
    ...
    def updateWeights(self,history,trainingExamples):
        for i in range(0,len(history)):
            ...
            h2 += self.trainingRate*(vTrain - vEst)*x2
            h3 += self.trainingRate*(vTrain - vEst)*x3
            h4 += self.trainingRate*(vTrain - vEst)*x4
            h5 += self.trainingRate*(vTrain - vEst)*x5
        ...
        for i in range(0,len(history)):
                ...
                if i >= len(history)-2: #last or second last iteration of history 
                    if(self.checker.getWinner(history[len(history)-1]) == 0): #current game doesn't have a winner
                        ...
                        trainingExamples.append([self.checker.getFeatures(history[i]), 100])
                    else: #the current game has a winner then its a bad thing?
                        trainingExamples.append([self.checker.getFeatures(history[i]), -100]) 
